...
class input_test(object):
    ...
    def run(self):
        ...
        #raw_window = pygame.display.get_surface()
        #raw_window.always_on_top = True
        
        # Get the window handle (HWND)
        hwnd = pygame.display.get_wm_info()['window']

        # Set the window to HWND_TOPMOST (-1)
        ...
        while True:
            ...
            for event in [pygame.event.wait(), ] + pygame.event.get():
                ...
                if event.type == QUIT:
                    #self.quit()
                    print('Press F12 to stop gamepad')
    ...
    def draw_joy(self, joyid):
        ...
        for l in range(joy.numbuttons // 11 + 1):
            s = []
            for i in range(l * 11, min((l + 1) * 11, joy.numbuttons)):
                if joy.button[i]:
                    s.append("%d" % (i % 11))
                else:
                    s.append(" ")
